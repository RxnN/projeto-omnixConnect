...
export const GET = withErrorHandling(async () => {
  ...
  const [empresa, filiais, users, products, pedidos, movements, promotions, suppliers, audit] = await Promise.all([
    ...
    prisma.supplier.findMany({ where: { empresaId: user.empresaId } }),
    prisma.tenantAuditLog.findMany({ where: { empresaId: user.empresaId } }),
  ]);
  const exportedAt = new Date().toISOString();
  return new NextResponse(JSON.stringify({ exportedAt, empresa, filiais, users, products, pedidos, movements, promotions, suppliers, audit }, null, 2), {
