...
describe("createPedido", () => {
  it("saída (OUT) diminui o estoque e soma o total corretamente", async () => {
    ...
    const updated = (await getProductById(product.id, filial.id))!;
...
describe("checkPedidoStock", () => {
  it("aponta itens cuja quantidade pedida excede o estoque disponível", async () => {
    ...
    const insufficient = await checkPedidoStock(filial.id, [{ productId: product.id, quantity: 8 }]);
  ...
  it("não aponta nada quando o estoque é suficiente", async () => {
    ...
    const insufficient = await checkPedidoStock(filial.id, [{ productId: product.id, quantity: 5 }]);
...
describe("cancelPedido", () => {
  it("cancelar uma saída devolve o estoque", async () => {
    ...
    expect((await getProductById(product.id, filial.id))!.currentStock).toBe(6);

    const cancelled = await cancelPedido(pedido.id, filial.id, user.id);
  ...
  it("cancelar uma entrada remove o estoque que ela havia adicionado", async () => {
    ...
    expect((await getProductById(product.id, filial.id))!.currentStock).toBe(12);

    await cancelPedido(pedido.id, filial.id, user.id);

    expect((await getProductById(product.id, filial.id))!.currentStock).toBe(0);
  });
